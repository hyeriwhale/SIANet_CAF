...
class sianet(nn.Module):
    def __init__(
        ...
        if dim == 2 and planar_blocks != ():
            ...
            # (hr)     
            #     dim=2: Conv2d with 3x3 kernel -> 어차피 2차원 커널 쓸 거라서 planar_blocks 설정 필요없음
            #     dim=3 and planar=False: Conv3d with 3x3x3 kernel -> 3차원 커널 사용
            #     dim=3 and planar=True: Conv3d with 1x3x3 kernel -> 2차원 커널 사용
        ...
        for i in range(n_blocks):
            ins = self.in_channels if i == 0 else outs  
              # (hr) 첫번째 블락이면 ins에 in_channels 사용, 그 후 블락이면 이전 loop에서 계산된 outs 사용
            outs = self.start_filts * (2**i)
              # (hr) 16 -> 32 -> 64 ...
            pooling = True if i < n_blocks - 1 else False
            planar = i in self.planar_blocks
              # (hr) i 값이 planar_blocks에 속한다면 planar 변수는 True가 되고, 속하지 않는다면 False가 됨
            ...
            self.down_convs.append(down_conv)  # (hr) 앞서 만들었던 빈 모듈 리스트에 하나씩 추가

        # create the decoder pathway and add to a list
        # - careful! decoding only requires n_blocks-1 blocks  # (hr) 인코더 블락: 5개, 디코더 블락: 4개
